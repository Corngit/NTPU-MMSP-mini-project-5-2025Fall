...
static int16_t clamp_i16(double x){
    if (x > 32767.0) return 32767;        // 超過 int16 最大值就截斷
    if (x < -32768.0) return -32768;      // 小於 int16 最小值就截斷
    return (int16_t)lrint(x);             // 四捨五入到最近整數（回傳 long int），再轉 int16
}
...
static double s_wave(int j, double t, double f_hz){
    ...
    switch (j){                           // j 代表每一秒的波形種類
        case 0:                           // j=0 是 sine 波
            return sin(2.0 * M_PI * f_hz * t); // sin(2πft)

        case 1:{                          // j=1 是 sawtooth 波，題目：fᵢt - floor(fᵢt)
            return ft - floor(ft);        // 取小數部分，範圍 [0,1)
        }

        case 2:{                          // j=2 是 square，題目：sgn(sin(2πft))
            double v = sin(2.0 * M_PI * f_hz * t); // 先算 sine
            if (v > 0.0) return  1.0;     // 正+1
            if (v < 0.0) return -1.0;     // 負-1
            return 0.0;                   // 0（邊界）
        }

        case 3:{                          // j=3 是 triangle ，題目：2*|2*(ft-floor(ft+0.5))|-1
            double inner = ft - floor(ft + 0.5);   // (ft - floor(ft+0.5))
            return 2.0 * fabs(2.0 * inner) - 1.0;  // 形成 [-1,1] 的三角波
        }
...
static int gen_and_save(uint32_t fs, const char *out_name){
    ...
    uint32_t total_samples = (uint32_t)llrint(total_sec * (double)fs);
    ...
    for (uint32_t n = 0; n < total_samples; n++){
        ...
        int i = (int)floor(t_in_sec / seg_sec); // 每秒的第幾段（0~9）
        if (i < 0) i = 0;                 // 保險 clamp
        if (i > 9) i = 9;
